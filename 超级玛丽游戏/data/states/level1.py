...
class Level1(tools._State):
    ...
    def setup_ground(self):
        ...
        sprites to walk on1988画的是地面，中间间隔是大坑"""
        ground_rect1 = collider.Collider(0, c.GROUND_HEIGHT,    780, 60)
    ...
    def setup_pipes(self):
        ...
        pipe1 = collider.Collider(1079, 680, 58, 140)
        pipe2 = collider.Collider(1320, 650, 58, 170)
        pipe3 = collider.Collider(1634, 650, 58, 170)

        pipe4 = collider.Collider(2504, 650, 58, 170)
        pipe5 = collider.Collider(2950, 710, 58, 82)

        pipe6 = collider.Collider(3588, 710, 58, 82)

        pipe7 = collider.Collider(4690, 710, 58, 82)

        self.pipe_group = pg.sprite.Group(pipe1,pipe2,pipe3,pipe4,pipe5,pipe6,pipe7)
    ...
    def setup_steps(self):
        ...
        step4 = collider.Collider(3891, 603, 32, 160)


        step5 = collider.Collider(4004, 603, 32, 160)
        step6 = collider.Collider(4036, 643, 32, 40)
        step7 = collider.Collider(4068, 683, 32, 40)
    ...
    def setup_enemies(self):
        ...
        enemy_group3 = pg.sprite.Group(goomba4)

        enemy_group4 = pg.sprite.Group(koopa0)
        enemy_group5 = pg.sprite.Group(goomba10, goomba11)
        enemy_group6 = pg.sprite.Group(goomba13)
    ...
    def update_all_sprites(self, keys):
        ...
        self.flag_pole_group1.update()
        self.flag_pole_group2.update()
        self.flag_pole_group3.update()
        self.flag_pole_group4.update()
    ...
    def check_points_check(self):
        ...
        if checkpoint:
            ...
            if checkpoint.name == '11':
                ...
                if self.mario.rect.bottom < self.flag.rect.y:
                    self.mario.rect.bottom = self.flag.rect.y
                self.flag1.state = c.SLIDE_DOWN
                self.create_flag_points(checkpoint.rect.right)

            elif checkpoint.name == 'flag2':
                self.mario.state = c.CENTER_FLAGPOLE
                # self.mario.invincible = False
                self.mario.flag_pole_right = checkpoint.rect.right
                if self.mario.rect.bottom < self.flag.rect.y:
                    self.mario.rect.bottom = self.flag.rect.y
                self.flag2.state = c.SLIDE_DOWN
                self.create_flag_points(checkpoint.rect.right)

            elif checkpoint.name == 'flag3':
                self.mario.state = c.CENTER_FLAGPOLE
                # self.mario.invincible = False
                self.mario.flag_pole_right = checkpoint.rect.right
                if self.mario.rect.bottom < self.flag.rect.y:
                    self.mario.rect.bottom = self.flag.rect.y
                self.flag3.state = c.SLIDE_DOWN
                self.create_flag_points(checkpoint.rect.right)

            elif checkpoint.name == 'flag4':
                self.mario.state = c.CENTER_FLAGPOLE
                # self.mario.invincible = False
                self.mario.flag_pole_right = checkpoint.rect.right
                if self.mario.rect.bottom < self.flag.rect.y:
                    self.mario.rect.bottom = self.flag.rect.y
                self.flag4.state = c.SLIDE_DOWN
                self.create_flag_points(checkpoint.rect.right)
    ...
    def check_flag(self):
        ...
        elif (self.flag.state == c.BOTTOM_OF_POLE
              and self.mario.state == c.CENTER_FLAGPOLE):
            self.mario.set_state_to_bottom_of_pole()
    ...
    def blit_everything(self, surface):
        ...
        self.flag_pole_group2.draw(self.level)
        self.flag_pole_group3.draw(self.level)
        self.flag_pole_group4.draw(self.level)
